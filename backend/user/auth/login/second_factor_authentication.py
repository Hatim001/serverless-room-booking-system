...
def validate_second_factor_authentication(email, question, answer):
    ...
    if not security_details.get("security_question")==question or not security_details.get("security_answer").lower()==answer.lower():
        return False
    else:
        return True
...
def lambda_handler(event, context):
    ...
    try:
        ...
        validation_result = validate_second_factor_authentication(email, question, answer)
        ...
        if validation_result:
            ...
            return prepare_response(status=200, message="Invalid security question or answer")
