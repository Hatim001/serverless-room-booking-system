...
def caesar_cipher_encryption(email, plain_text):
...
def lambda_handler(event, context):
    ...
    try:
        ...
        result = caesar_cipher_encryption(email, plain_text)
        ...
        if validation_message:
            ...
            if mfa_status:
                ...
                return prepare_response(
                    status=200,
                    message="MFA1 and MFA2 not passed",
                )
        else:
            return prepare_response(status=200, message="Encryption validation failed")
