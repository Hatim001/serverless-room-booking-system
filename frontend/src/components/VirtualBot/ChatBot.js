...
const ChatBot = ({ session }) => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      {!isOpen && (
        <button
          className="bg-blue-600 border-b-black text-white rounded-full p-4 shadow-lg"
        ...
        <div className="flex flex-col w-96 h-[450px] bg-white border border-gray-300 rounded-lg shadow-lg">
          ...
          <div className="flex-grow p-4 overflow-y-auto w-full">
            {messages.map((message, index) => (
              ...
              >
                <p  className={`mb-2 p-2  rounded w-fit ${
                  message.type === 'user' ? 'bg-gray-200 text-gray-700 ' : ' bg-blue-200 text-gray-700'
                }`}>{message.text}</p>
              </div>
            ))}
          </div>
          ...
          <div className="flex p-4 border-t border-gray-300">
            ...
            />
            <button onClick={handleSendMessage} className="bg-blue-600 text-white rounded-full px-4 py-2">
              <Send />
            </button>
          </div>
        </div>
