...
export async function sendMessageToDialogflow(message, session) {
    ...
    queryParams: {
      parameters: {
        fields: {
          userRole: {
            kind: 'stringValue',
            stringValue: session.role ? session.role : 'guest',
          },
          userEmail: {
            kind: 'stringValue',
            stringValue: session.user.email ? session.user.email : '',
          },
        },
      },
    },
  ...
  try {
    ...
    const messages = responses[0].queryResult.responseMessages.flatMap(
